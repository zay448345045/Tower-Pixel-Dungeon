...
import com.fixakathefix.towerpixeldungeon.windows.WndSettings;
import com.fixakathefix.towerpixeldungeon.windows.WndStartGame;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnJournal = new StyledButton(GREY_TR, Messages.get(this, "journal")){
			@Override
			protected void onClick() {
				ShatteredPixelDungeon.switchNoFade( JournalScene.class );
			}
		};
		btnJournal.icon(Icons.get(Icons.COPY));
		add(btnJournal);
		...
		btnSupport.icon(Icons.get(Icons.FIX));
		add(btnSupport);
		...
		if (landscape()) {
			...
			align(btnPlay);
			btnJournal.setRect(btnPlay.right()+2, btnPlay.top(), btnPlay.width(), BTN_HEIGHT);
			...
			btnRankings.setRect(btnPlay.left(), btnJournal.bottom()+ GAP, (btnPlay.width()/2)-1, BTN_HEIGHT);
