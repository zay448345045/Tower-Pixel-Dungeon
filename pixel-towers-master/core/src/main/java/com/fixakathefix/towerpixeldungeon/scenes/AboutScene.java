...
import com.fixakathefix.towerpixeldungeon.ui.ScrollPane;
...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		CreditsBlock fix = new CreditsBlock(true, 0xEEAA33,
				...
				null);
		if (landscape()){
			fix.setRect((w - fullWidth)/2f - 6, 10, 120, 0);
		} else {
			fix.setRect((w - fullWidth)/2f, 6, 120, 0);
		}
		content.add(fix);
		...
		CreditsBlock addit = new CreditsBlock(false, 0x22DD33,
				"Additional",
				Icons.FIX.get(),
				"\nAdditional music by: `Jivvy` (YAPD soundtrack)\nAdditional sprites & designs: _FixAkaTheFix_",
				null,
				null);
		addit.setSize(colWidth, 0);
		if (landscape()){
			addit.setPos(fix.right(), fix.top());
		} else {
			addit.setPos(w/2f - colWidth/2f, fix.bottom()+5);
		}
		content.add(addit);
		...
		CreditsBlock shpx = new CreditsBlock(true, Window.SHPX_COLOR,
				...
				shpxLink);
		if (landscape()){
			shpx.setRect((w - fullWidth)/2f - 6, 14 + fix.bottom(), 120, 0);
		} else {
			shpx.setRect((w - fullWidth)/2f, 14 + addit.bottom(), 120, 0);
		}
		content.add(shpx);

		addLine(shpx.top() - 7, content);
