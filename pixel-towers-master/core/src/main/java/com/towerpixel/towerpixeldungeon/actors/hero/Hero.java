...
import com.towerpixel.towerpixeldungeon.actors.buffs.AdrenalineSurge;
...
import com.towerpixel.towerpixeldungeon.actors.buffs.Awareness;
...
import com.towerpixel.towerpixeldungeon.items.Heap.Type;
...
import com.towerpixel.towerpixeldungeon.items.artifacts.CapeOfThorns;
...
import com.towerpixel.towerpixeldungeon.items.keys.Key;
...
import com.towerpixel.towerpixeldungeon.scenes.AlchemyScene;
import com.towerpixel.towerpixeldungeon.scenes.GameScene;
import com.towerpixel.towerpixeldungeon.scenes.InterlevelScene;
import com.towerpixel.towerpixeldungeon.scenes.SurfaceScene;
...
import com.towerpixel.towerpixeldungeon.ui.StatusPane;
...
public class Hero extends Char {
	...
	public float critMult() {
		if (hero.belongings.weapon != null) return hero.belongings.weapon.critMult * hero.critMult + RingOfLethality.critMultAdd(this);
		else return hero.critMult + RingOfLethality.critMultAdd(this);
	}
	@Override
	public float critChance() {
		if (hero.belongings.weapon != null) return hero.belongings.weapon.critChance + hero.critChance + RingOfLethality.critChanceAdd(this);
		else return hero.critChance + RingOfLethality.critChanceAdd(this);
	}
	...
	public boolean canSurpriseAttack(){
		...
		if (w instanceof DualHatchet)           return false;
	...
	private boolean getCloser( final int target ) {
		...
		if (target == pos)
			return false;
		if (Dungeon.level.herounpassable[target]) return false;//main thing to stop the hero from surpassing tiles with the HEROUNPASSABLE flag
	...
	public void earnExp( int exp, Class source ) {
		...
		while (this.exp >= maxExp()) {
			...
			if (lvl < MAX_LEVEL) {
				...
				updateHT( true );
				//attackSkill++; Nah Those wont let me spam rats in the endgame
				//defenseSkill++;
