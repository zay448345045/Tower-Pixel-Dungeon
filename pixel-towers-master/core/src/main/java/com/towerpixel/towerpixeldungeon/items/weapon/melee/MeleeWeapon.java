...
import com.towerpixel.towerpixeldungeon.actors.buffs.LockedFloor;
...
import com.towerpixel.towerpixeldungeon.ui.HeroIcon;
...
public class MeleeWeapon extends Weapon {
	...
	public String info() {
		...
		if (levelKnown) {
			info += "\n\n" + Messages.get(MeleeWeapon.class, "stats_known", tier, augment.damageFactor(min()), augment.damageFactor(max()), STRReq(), Math.round(100*(this.critChance+hero.critChance())), (Math.round(100*(hero.critMult()*this.critMult)))-100);
			...
			info += "\n\n" + Messages.get(MeleeWeapon.class, "stats_unknown", tier, min(0), max(0), STRReq(0), Math.round(100*(this.critChance+hero.critChance())), (Math.round(100*(this.critMult*hero.critMult())))-100);
